...
/** Class encapsulating EULO Core startup and shutdown.
...
{
    QMessageBox::critical(0, "Runaway exception", BitcoinGUI::tr("A fatal error occurred. EULO can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    ...
    Q_INIT_RESOURCE(eulo_locale);
    Q_INIT_RESOURCE(eulo);
    ...
    /// 6. Determine availability of data directory and parse eulo.conf
    ...
    try {
        ...
        WinShutdownMonitor::registerShutdownBlockReason(QObject::tr("EULO Core didn't yet exit safely..."), (HWND)app.getMainWinId());
#endif
        app.exec();
        app.requestShutdown();
        app.exec();
