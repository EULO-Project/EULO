...
static std::string ValueToString(CAmount nValue, bool AllowNegative = false)
{
    ...
    QString Str = BitcoinUnits::formatWithUnit(BitcoinUnits::ULO, nValue);
...
{
    std::string genesisblockhash = "00000013ea705af4ed8b40a86250bc6d78b982c604eef80a908cec7f11abbdd1";
...
void BlockExplorer::showEvent(QShowEvent*)
{
    if (m_NeverShown) {
        ...
        if (!GetBoolArg("-txindex", false)) {
            QString Warning = tr("Not all transactions will be shown. To view all transactions you need to set txindex=1 in the configuration file (eulo.conf).");
            QMessageBox::warning(this, "EULO Core Blockchain Explorer", Warning, QMessageBox::Ok);
        }
    }
}
