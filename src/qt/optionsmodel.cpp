...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeEuloAmount"))
        settings.setValue("nAnonymizeEuloAmount", 1000);

    nAnonymizeEuloAmount = settings.value("nAnonymizeEuloAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeEuloAmount"))
        SoftSetArg("-anonymizeeuloamount", settings.value("nAnonymizeEuloAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in eulo.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeEuloAmount:
            return QVariant(nAnonymizeEuloAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeEuloAmount:
            nAnonymizeEuloAmount = value.toInt();
            settings.setValue("nAnonymizeEuloAmount", nAnonymizeEuloAmount);
            emit anonymizeEuloAmountChanged(nAnonymizeEuloAmount);
            break;
        case CoinControlFeatures:
