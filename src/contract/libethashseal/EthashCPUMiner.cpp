...
void EthashCPUMiner::startWorking()
{
    ...
    {
        m_shouldStop = false;
        m_thread.reset(new boost::thread(&EthashCPUMiner::minerBody, this));
    }
}
...
void EthashCPUMiner::minerBody()
{
    ...
    static std::mt19937_64 s_eng((utcTime() + boost::hash<decltype(tid)>()(tid)));
...
{
    string baseline = toString(boost::thread::hardware_concurrency()) + "-thread CPU";
    return baseline;
}
