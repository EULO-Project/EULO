...
public:
    ...
    EuloDGP(EuloState *_state, bool _dgpevm = true) : dgpevm(_dgpevm), state(_state)
    {
        initDataEIP158();
    }
...
private:
    ...
    const EuloState *state;

    dev::Address templateContract;

    std::map<dev::h256, std::pair<dev::u256, dev::u256>> storageDGP; //eulo hashedKey,<key,value>

    std::map<dev::h256, std::pair<dev::u256, dev::u256>> storageTemplate;//eulo hashedKey,<key,value>

    std::vector<unsigned char> dataTemplate;   //eulo call contract ,res->output
