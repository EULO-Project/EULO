...
void EuloDGP::initDataEIP158()
{
    std::vector<uint32_t> tempData = {dev::eth::EIP158Schedule.tierStepGas[0], dev::eth::EIP158Schedule.tierStepGas[1],
                                      dev::eth::EIP158Schedule.tierStepGas[2],
                                      dev::eth::EIP158Schedule.tierStepGas[3], dev::eth::EIP158Schedule.tierStepGas[4],
                                      dev::eth::EIP158Schedule.tierStepGas[5],
                                      dev::eth::EIP158Schedule.tierStepGas[6], dev::eth::EIP158Schedule.tierStepGas[7],
                                      dev::eth::EIP158Schedule.expGas,
                                      dev::eth::EIP158Schedule.expByteGas, dev::eth::EIP158Schedule.sha3Gas,
                                      dev::eth::EIP158Schedule.sha3WordGas,
                                      dev::eth::EIP158Schedule.sloadGas, dev::eth::EIP158Schedule.sstoreSetGas,
                                      dev::eth::EIP158Schedule.sstoreResetGas,
                                      dev::eth::EIP158Schedule.sstoreRefundGas, dev::eth::EIP158Schedule.jumpdestGas,
                                      dev::eth::EIP158Schedule.logGas,
                                      dev::eth::EIP158Schedule.logDataGas, dev::eth::EIP158Schedule.logTopicGas,
                                      dev::eth::EIP158Schedule.createGas,
                                      dev::eth::EIP158Schedule.callGas, dev::eth::EIP158Schedule.callStipend,
                                      dev::eth::EIP158Schedule.callValueTransferGas,
                                      dev::eth::EIP158Schedule.callNewAccountGas,
                                      dev::eth::EIP158Schedule.suicideRefundGas, dev::eth::EIP158Schedule.memoryGas,
                                      dev::eth::EIP158Schedule.quadCoeffDiv, dev::eth::EIP158Schedule.createDataGas,
                                      dev::eth::EIP158Schedule.txGas,
                                      dev::eth::EIP158Schedule.txCreateGas, dev::eth::EIP158Schedule.txDataZeroGas,
                                      dev::eth::EIP158Schedule.txDataNonZeroGas,
                                      dev::eth::EIP158Schedule.copyGas, dev::eth::EIP158Schedule.extcodesizeGas,
                                      dev::eth::EIP158Schedule.extcodecopyGas,
                                      dev::eth::EIP158Schedule.balanceGas, dev::eth::EIP158Schedule.suicideGas,
                                      dev::eth::EIP158Schedule.maxCodeSize};
    dataEIP158Schedule = tempData;
}

bool EuloDGP::checkLimitSchedule(const std::vector<uint32_t> &defaultData, const std::vector<uint32_t> &checkData)
{
...
dev::eth::EVMSchedule EuloDGP::getGasSchedule(unsigned int blockHeight)
{
    clear();
...
uint64_t
EuloDGP::getUint64FromDGP(unsigned int blockHeight, const dev::Address &contract, std::vector<unsigned char> data)
{
    uint64_t value = 0;
...
uint32_t EuloDGP::getBlockSize(unsigned int blockHeight)
{
    clear();
...
uint64_t EuloDGP::getMinGasPrice(unsigned int blockHeight)
{
    clear();
...
uint64_t EuloDGP::getBlockGasLimit(unsigned int blockHeight)
{
    clear();
...
bool EuloDGP::initStorages(const dev::Address &addr, unsigned int blockHeight, std::vector<unsigned char> data)
{
    initStorageDGP(addr);
...
void EuloDGP::initStorageDGP(const dev::Address &addr)
{
    storageDGP = state->storage(addr);
}

void EuloDGP::initStorageTemplate(const dev::Address &addr)
{
    storageTemplate = state->storage(addr);
}

extern std::vector<ResultExecute> CallContract(const dev::Address &addrContract, std::vector<unsigned char> opcode,
                                               const dev::Address &sender = dev::Address(), uint64_t gasLimit = 0);

void EuloDGP::initDataTemplate(const dev::Address &addr, std::vector<unsigned char> &data)
{
    dataTemplate = CallContract(addr, data)[0].execRes.output;
}

void EuloDGP::createParamsInstance()
{
...
dev::Address EuloDGP::getAddressForBlock(unsigned int blockHeight)
{
...
void EuloDGP::parseStorageScheduleContract(std::vector<uint32_t> &uint32Values)
{
...
void EuloDGP::parseDataScheduleContract(std::vector<uint32_t> &uint32Values)
{
    size_t size = dataTemplate.size() / 32;
    for (size_t i = 0; i < size; i++)
    {
        std::vector<unsigned char> value = std::vector<unsigned char>(dataTemplate.begin() + (i * 32),
...
void EuloDGP::parseStorageOneUint64(uint64_t &value)
{
...
void EuloDGP::parseDataOneUint64(uint64_t &value)
{
...
dev::eth::EVMSchedule EuloDGP::createEVMSchedule()
{
    dev::eth::EVMSchedule schedule = dev::eth::EIP158Schedule;
