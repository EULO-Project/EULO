...
using ExtractEuloTX = std::pair<std::vector<EuloTransaction>, std::vector<EthTransactionParams>>;


class EuloTxConverter
{

public:

    EuloTxConverter(CTransaction tx, CCoinsViewCache *v = NULL, const std::vector<CTransaction> *blockTxs = NULL)
            : txBit(tx), view(v), blockTransactions(blockTxs)
    {
    }

    bool extractionEuloTransactions(ExtractEuloTX &euloTx);
...
private:
    ...
    EuloTransaction createEthTX(const EthTransactionParams &etp, const uint32_t nOut);
...
public:
    ...
    ByteCodeExec(const CBlock &_block, std::vector<EuloTransaction> _txs, const uint64_t _blockGasLimit) : txs(_txs),
                                                                                                           block(_block),
                                                                                                           blockGasLimit(
                                                                                                                   _blockGasLimit)
    {
...
private:
    ...
    std::vector<EuloTransaction> txs;
