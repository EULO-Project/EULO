...
QHash<int, QByteArray> TokenTransactionTableModel::roleNames() const {
...
QVariant TokenTransactionTableModel::data(const QModelIndex &index, int role) const
{
    ...
    case AddressRole_:
        return formatTxToAddress(rec, false) + "|" +QString::number((int)(rec->status.status));
    case NameRole_:
        return formatTxTokenSymbol(rec) + "|" +QString::number((int)(rec->status.status));
    case AmountRole_:
