...
{
    CTxDestination qtumAddress = CBitcoinAddress(strQtumAddress).Get();

    if(qtumAddress.which() == 0)
        return false;
...
bool ToQtumAddress(const std::string& strHash160, std::string& strQtumAddress)
{
    ...
    if(qtumAddress.which() != 0){
        strQtumAddress = CBitcoinAddress(qtumAddress).ToString();
        return true;
    }
    return false;
}
...
bool Token::name(std::string &result, bool sendTo)
{
    ...
    if(!exec(input, d->funcName, output, sendTo, errorMessage))
        return false;
...
bool Token::approve(const std::string &_spender, const std::string &_value, bool &success, bool sendTo)
{
    ...
    if(!exec(input, d->funcApprove, output, sendTo,errorMessage))
        return false;
...
bool Token::totalSupply(std::string &result, bool sendTo)
{
    ...
    if(!exec(input, d->funcTotalSupply, output, sendTo,errorMessage))
        return false;
...
bool Token::transferFrom(const std::string &_from, const std::string &_to, const std::string &_value, bool &success, bool sendTo)
{
    ...
    if(!exec(input, d->funcTransferFrom, output, sendTo,errorMessage))
        return false;
...
bool Token::decimals(std::string &result, bool sendTo)
{
    ...
    if(!exec(input, d->funcDecimals, output, sendTo,errorMessage))
        return false;
...
bool Token::burn(const std::string &_value, bool &success, bool sendTo)
{
    ...
    if(!exec(input, d->funcBurn, output, sendTo,errorMessage))
        return false;
...
bool Token::balanceOf(const std::string &spender, std::string &result, bool sendTo)
{
    ...
    if(!exec(input, d->funcBalanceOf, output, sendTo,errorMessage))
        return false;
...
bool Token::burnFrom(const std::string &_from, const std::string &_value, bool &success, bool sendTo)
{
    ...
    if(!exec(input, d->funcBurnFrom, output, sendTo,errorMessage))
        return false;
...
bool Token::symbol(std::string &result, bool sendTo)
{
    ...
    if(!exec(input, d->funcSymbol, output, sendTo,errorMessage))
        return false;
...
bool Token::transfer(const std::string &_to, const std::string &_value, QString &errorMessage, bool sendTo)
{
    std::string to = _to;
    if(!ToHash160(to, to))
    {
    ...
    return exec(input, d->funcTransfer, output, sendTo, errorMessage);
}
...
bool Token::approveAndCall(const std::string &_spender, const std::string &_value, const std::string &_extraData, bool &success, bool sendTo)
{
    ...
    if(!exec(input, d->funcApproveAndCall, output, sendTo,errorMessage))
        return false;
...
bool Token::allowance(const std::string &_from, const std::string &_to, std::string &result, bool sendTo)
{
    ...
    if(!exec(input, d->funcAllowance, output, sendTo,errorMessage))
        return false;
...
bool Token::exec(const std::vector<std::string> &input, int func, std::vector<std::string> &output, bool sendTo, QString &errorMessage)
{
