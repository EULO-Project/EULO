                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zULO ought to be enough for anybody." - Bill Gates, 2017
...
void PrivacyDialog::on_pushButtonSpendzULO_clicked()
{
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zULO
        sendzULO();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zULO
    sendzULO();
}
...
void PrivacyDialog::sendzULO()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Eulo Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    // Warn for additional fees if amount is not an integer and change as zULO is requested
    ...
    // use mints from zUlo selector if applicable
