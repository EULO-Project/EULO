...
public:
    ...
    bool transfer(const std::string& _to, const std::string& _value,QString &errorMessage, bool sendTo = false);
...
private:
    bool exec(const std::vector<std::string>& input, int func, std::vector<std::string>& output, bool sendTo,QString &errorMessage);
