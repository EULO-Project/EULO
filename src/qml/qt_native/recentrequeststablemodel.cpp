...
QHash<int, QByteArray> RecentRequestsTableModel::roleNames() const
{
...
QVariant RecentRequestsTableModel::data(const QModelIndex& index, int role) const
{
    ...
    {
        if (rec->recipient.label.isEmpty()) {
            return tr("(no label)");
        } else {
            return rec->recipient.label;
        }
    }
    else if (role == LabelRole_)
    {
        return rec->recipient.label;
    }
    ...
    else if (role == MessageRole_)
    {
        return rec->recipient.message;
    }
    else if (role == AmountRole)
    {
        if (rec->recipient.amount == 0)
            return tr("(no amount)");
        else
            return BitcoinUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), rec->recipient.amount);
    }
    else if (role == AmountRole_)
    {
        return (quint64)(rec->recipient.amount);
    }
...
void RecentRequestsTableModel::updateAmountColumnTitle()
{
    ...
    emit dataChanged(index(0,Amount),index(rowCount(QModelIndex())-1,Amount));
