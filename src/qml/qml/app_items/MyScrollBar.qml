...
T.ScrollBar {
    ...
    Rectangle {
           ...
           // opacity: handle.opacity
            width:handle.width
            height:handle.height<=30?30:handle.height
        }
    ...
    transitions: [
        Transition {
            to: "active"
            NumberAnimation { targets: [handle2, background]; property: "opacity"; to: 1.0 }
        },
        Transition {
            from: "active"
            SequentialAnimation {
                PropertyAction{ targets: [handle2, background]; property: "opacity"; value: 1.0 }
                PauseAnimation { duration: 2450 }
                NumberAnimation { targets: [handle2, background]; property: "opacity"; to: 0.0 }
            }
        }
    ]
}
